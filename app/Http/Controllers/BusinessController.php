...
use App\Notifications\SuperadminCommunicator;
...
use app\SuperadminCommunicatorLog;
use App\Variation;
...
class BusinessController extends Controller
{
    ...
    */
    ...
    //         DB::beginTransaction();
    //         // $url_master = env('MASTER_URL');
    //         // $tenant_master = Tenant::where('domain', $url_master)->where('name', 'Admin')->first();
    //         // $tenantService = new TenantServcie();
    //         // $tenantService->switchToTenant($tenant_master);
    //         $user = $this->createUser($request);

    //         $business = $this->createBusiness($request, $user);
    //         DB::commit();
    //         DB::beginTransaction();
    //         $this->handleTenantRegistration($request, $user, $business);
    //         $user = $this->createUser($request);

    //         $business = $this->createBusiness($request, $user);
    //         $tenantService = new TenantServcie();
    //         $tenantService->switchToDefault();
    //         $tenant_con = Tenant::where('name', $request->username)->first();
    //         info('tenant_con: ' . $tenant_con);
    //         $subdomain = $tenant_con->domain;
    //         info('Subdomain: ' . $subdomain);
    //         DB::commit();
    //         // DB::connection('support')->table('customers')->insert([
    ...
    //         return response()->json([
    //             'success' => true,
    //             'message' => __('business.business_created_succesfully'),
    //             'redirect_domain' => $subdomain,
    //         ], 200);
    //     } catch (\Exception $e) {
    //         DB::rollBack();
    ...
    //             'password.confirmed' => __('validation.confirmed', ['attribute' => __('business.username')]),
    ...
    // {
    //     $ownerDetails = $request->only(['surname', 'first_name', 'last_name', 'username', 'email', 'password', 'language']);
    //     $ownerDetails['language'] = $ownerDetails['language'] ?: config('app.locale');

    //     return User::create_user($ownerDetails);
    // }

    // protected function createBusiness(Request $request, User $user)
    // {
    //     $businessDetails = $request->only([
    ...
    //         $businessDetails['start_date'] = Carbon::createFromFormat(
    //             config('constants.default_date_format'),
    //             $businessDetails['start_date']
    //         )->toDateString();
    //     }

    //     $logoName = $this->businessUtil->uploadFile($request, 'business_logo', 'business_logos', 'image');
    //     $businessDetails['logo'] = $logoName ?: null;

    //     $businessDetails['enabled_modules'] = ['purchases', 'add_sale', 'pos_sale', 'expenses'];

    //     return $this->businessUtil->createNewBusiness($businessDetails);
    // }
    ...
    public function postRegister(Request $request)
    {
        ...
        try {
            ...
            // $business_details['enabled_modules'] = ['purchases', 'add_sale', 'pos_sale', 'expenses'];
            ...
            // $admin = User::find(1);
            // $admin->notify(new AdminRegistrationNotification($user, $business));
            ...
            // $tenant = Tenant::where('name', $request->username)->first();
            // $tenantService = new TenantServcie();
            ...
            $tenantService = new TenantServcie();
            $tenantService->switchToDefault();
            $tenant_con = Tenant::where('name', 'Admin')->first();

            $result = $this->handleUserRegistration($request, $tenant_con, 'main');
            $tenant_user = $this->handleTenantRegistration($request,  $result['user'],  $result['business']);
            $result = $this->handleUserRegistration($request, $tenant_user, 'user');
            ...
            $subdomain = $tenant_con->domain;
            info('Subdomain: ' . $subdomain);
            error_log('tenant_con: ' . print_r($tenant_con, true));
    ...
    protected function handleTenantRegistration(Request $request, User $user, $business)
    {
        try {
            $tenantService = new TenantServcie();
            $tenantService->switchToDefault();

            $nameUsing = str_replace(' ', '-', strtolower($request->username));
            $nameUsingDB = str_replace(' ', '_', strtolower($request->username));
            $subdomainName = $nameUsing;

            $cpanelDomain = env('CPANEL_DOMAIN', 'restaurant.upgrade-ai.net');
            $cpanelApiToken = env('CPANEL_API_TOKEN', 'HBL5YQO084WP9Y2AGO05TVUEPYYUYY6K');
            $cpanelUsername = env('CPANEL_USERNAME', 'restaupgrad');
            $cpanelPort = env('CPANEL_PORT', '2083');
            $path = '/public_html';

            if (env('CPANEL') == 'cpanel') {
                $cpanel = new Cpanel($cpanelDomain, $cpanelApiToken, $cpanelUsername, 'https', $cpanelPort);

                $databaseName = $cpanelUsername . '_' . $nameUsingDB;
                $username = $cpanelUsername . '_' . $nameUsingDB;
                $password = $this->generateRandomPassword();

                // Database operations
                $cpanel->createSubDomain($subdomainName, $cpanelDomain, $path);
                $cpanel->createDatabase($databaseName);
                $cpanel->createDatabaseUser($username, $password);
                $cpanel->setAllPrivilegesOnDatabase($databaseName, $username);


                // Cloudflare operations
                $authorization = env('AUTHORIZATION', 'J1-q3w8sSUfnUdrR6DTcZnwnM4sAyq3hpCKVyPWS');
                $zoneId = env('ZONE_ID', '691211f7c0cd9769eaf872890d7ad264');
                $cloudflareContent = env('CLOUDFLARE_CONTENT', '199.192.22.90');
                $basicName = env('BASIC_NAME', 'restaurant');
                $cloudflareName = $subdomainName . '.' . $basicName;
                ...
                if ($response->failed()) {
                    throw new \Exception('Cloudflare API Request failed: ' . $response->status());
                }
            } else {
                $databaseName = $cpanelUsername . '_' . $nameUsingDB;
            ...
            $domain = $subdomainName . '.' . $cpanelDomain;
            ...
            // Switch to the newly created tenant and run migrations and seed
            $tenantService->switchToTenant($tenants);
            // Tenants::switchToTenant($tenants);
            info('start migrate : ' . now());
            // retry(3, function () {
            Artisan::call('migrate --database=tenant');
            info('End migrate : ' . now());
            info('start seed : ' . now());
            ...
            // Optionally, return a response or take appropriate action.
        }
        return $tenants;
    }
    ...
    protected function handleUserRegistration(Request $request, $tenant, $type)
    {
        $tenantService = new TenantServcie();
        $tenantService->switchToTenant($tenant);
        info('Start : ' . $type);
    ...
    public function getBusinessSettings()
    {
        ...
        if (auth()->user()->can('superadmin')) {
            $modules = $this->moduleUtil->availableModulesSuperadmin();
            $modules_report = $this->moduleUtil->availableReports();
        } else {
        ...
        return view('business.settings', compact('business', 'currencies', 'tax_rates', 'timezone_list', 'months', 'accounting_methods', 'commission_agent_dropdown', 'units_dropdown', 'date_formats', 'shortcuts', 'pos_settings', 'modules', 'theme_colors', 'email_settings', 'sms_settings', 'mail_drivers', 'allow_superadmin_email_settings', 'custom_labels', 'common_settings', 'weighing_scale_setting', 'payment_types', 'modules_report'));
    }
